...
import numpy as np
from lib.utils import load_graphdata, compute_val_loss_single, evaluate_on_test_single
from lib.metrics import masked_mae_torch
from model.GRU import Enc_Dec
import shutil
...
parser.add_argument("--config", default="configurations/GRU.conf", type=str,
...
net = Enc_Dec(num_for_predict, L_tilde_node=L_tilde_node, dim_in_node=1, dim_out_node=1, adj_sub_edge=adj_edge_sub,
              L_tilde_edge=L_tilde_edge, dim_in_edge=1, dim_out_edge=1, range_K=K, types_accident=types_accident,
...
def train_main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_index in range(batch_train):
            ...
            optimizer.zero_grad()
            out_edge = net(inputs_node=train_x_node_tensor, hidden_state_node=None,
            ...
            loss = masked_mae_torch(out_edge, train_target_edge_tensor)
            loss.backward()
        ...
        evaluate_on_test_single(net, epoch, params_path, test_dataset, mean_node, std_node, mean_edge, std_edge,
                                  batch_size, DEVICE)
        val_loss = compute_val_loss_single(net, epoch, val_dataset, mean_node, std_node, mean_edge, std_edge,
                                             batch_size, DEVICE)
...
if __name__ == "__main__":
    train_main()
    # evaluate_on_test_single(net, 5, params_path, test_dataset, mean_node, std_node, mean_edge, std_edge,
    #                         batch_size, DEVICE)
